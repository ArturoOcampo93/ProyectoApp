...
if (isset($_POST['email']) && isset($_POST['nombre']) && isset($_POST['imagen'])) {
	//verifica vigencia
	if($vigencia){
		...
		if($valid){
			...
			$existe=Usuarios::buscaRegistroDisfraz($data['email']);
			if($existe['encontrado']=="si"){
				//ya existe el usuario
				$response["error_msg"]="Ya existe un usuario con este email.";
			}else{
				$response["error_msg"]="Felicidades tu registro fue existoso.";
				Usuarios::regDisfraz($data);
